<template>
  <div id="appEditPage">
    <div class="page-header"><h1>编辑应用信息</h1></div>
    <div class="edit-container">
      <a-card title="基本信息" :loading="loading">
        <a-form :model="formData" :rules="rules" layout="vertical" @finish="handleSubmit" ref="formRef">
          ...
          <a-form-item label="初始提示词" name="initPrompt">
            <a-textarea v-model:value="formData.initPrompt" :rows="4" :maxlength="1000" show-count disabled />
      ...
      <a-card title="应用信息" style="margin-top: 24px">
        <a-descriptions :column="2" bordered>
          ...
          <a-descriptions-item label="创建者"><UserInfo :user="appInfo?.user" size="small" /></a-descriptions-item>
          ...
          <a-descriptions-item label="部署时间">{{ appInfo?.deployedTime ? formatTime(appInfo.deployedTime) : '未部署' }}</a-descriptions-item>
...
const isAdmin = computed(() => String(loginUserStore.loginUser.userRole ?? '') === 'admin')
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      if (!isAdmin.value && appInfo.value.userId !== loginUserStore.loginUser.id) {
        message.error('您没有权限编辑此应用'); router.push('/'); return
      }
    ...
    } else { message.error('获取应用信息失败'); router.push('/') }
...
const handleSubmit = async () => {
  ...
  try {
    let res
    if (isAdmin.value) {
      res = await updateAppByAdmin({ id: appInfo.value.id as any, appName: formData.appName, cover: formData.cover, priority: formData.priority })
    } else {
      res = await updateApp({ id: appInfo.value.id as any, appName: formData.appName })
    }
...
<style scoped>
#appEditPage { padding: 24px; max-width: 1000px; margin: 0 auto; }
.page-header { display: flex; align-items: center; gap: 12px; margin-bottom: 24px; }
.page-header h1 { margin: 0; font-size: 24px; font-weight: 600; }
.edit-container { border-radius: 8px; }
.cover-preview { margin-top: 12px; padding: 12px; border: 1px solid #e8e8e8; border-radius: 6px; background: #fafafa; }
.form-tip { font-size: 12px; color: #999; margin-top: 4px; }
::deep(.ant-card-head) { background: #fafafa; }
::deep(.ant-descriptions-item-label) { background: #fafafa; font-weight: 500; }
</style>
